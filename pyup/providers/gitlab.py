...
class Provider(object):
    ...
    def _api(self, token):
        ...
        if len(parts) == 1:
            ...
            raise BadTokenError(
                ...
                'gitlab.com, or "apikey@https://yourgitlab.local"'.format(token)
            )
    ...
    def get_file(self, repo, path, branch):
        ...
        try:
            ...
            if e.response_code == 404:
                logger.warning("Unable to get {path}".format(path=path,))
                return None, None
        else:
    ...
    def create_branch(self, repo, base_branch, new_branch):
        try:
            repo.branches.create({"branch": new_branch, "ref": base_branch})
    ...
    def get_pull_request_committer(self, repo, pull_request):
        return [
            ...
            for participant in repo.mergerequests.get(
                pull_request.number
            ).participants()
        ]
    ...
    def create_pull_request(
        self, repo, title, body, base_branch, new_branch, pr_label, assignees, config
    ):
            ...
            if e.response_code == 409:
                ...
                for mr in repo.mergerequests.list(state="opened", all=True):
                    if (
                        ...
                        and mr.target_branch == base_branch
                    ):
                        mr.notes.create({"body": comment})
                        return self.bundle.get_pull_request_class()(
                            state=mr.state,
    ...
    def create_issue(self, repo, title, body):
        return repo.issues.create({"title": title, "description": body})
